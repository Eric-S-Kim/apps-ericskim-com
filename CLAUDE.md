...
5. After any change, verify the rendered published surface, not just local files. Tile LOOKS (name/icon/colors/glow) live in each device's localStorage and in `C:/code/cos-phone-sync/device-tiles.json` (private canon) — repo changes must never alter them. Since 2026-09-22 a device that holds the private config key re-syncs its registry tiles from the owner's private config service on every open (see "Remote tile sync" in `index.html`; test: `python scripts/launcher-sync.test.py`), so a tile change = edit the private canon + publish it, not a setup link. Setup links remain the fallback for a device without the key.
6. **Minimal chrome (every app in this store).** Keep each app as simple as possible: never add informational lines at the top or bottom of a screen (footers, "last updated"/"page checked" times, build or version strings, refresh notes, counts, source/debug notes, taglines, install hints) unless the owner explicitly designs that information in. Freshness or failure that matters goes in a designed element (for example a header age, or an alert line shown only when something is wrong); diagnostics belong in response headers or endpoints, not on screen.
