...
The main view shows events that have not ended. Overnight events stay visible until their end time; all-day events stay through their date. Consecutive days with the same title, exact original sources and an order identity share one date-range card, with each day's schedule expandable inside it. Regular classes and distinct orders stay separate. Grouping changes presentation only: every original and daily cancellation decision remains intact, and mixed day states cannot claim a ready ticket.
...
Offline installation requires the essential app and PDF viewer. Optional PDF fonts and image codecs are cached independently with a bounded wait; one unavailable optional asset cannot prevent the app from opening offline. An unsupported PDF can still be opened from its retained original.
